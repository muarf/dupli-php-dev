      ...
      $.get('?admin&ajax=get_change&id=' + id)
        .done(function (response) {
          ...
          if (response.success) {
            ...
            showAppModal({ type: 'danger', message: 'Erreur lors du chargement du changement: ' + (response.error || 'Erreur inconnue') });
          }
        })
        .fail(function (xhr, status, error) {
          console.error('Erreur AJAX:', xhr.responseText);
          showAppModal({ type: 'danger', message: 'Erreur lors du chargement du changement: ' + error });
        });
    });
    ...
    $('#save-edit').click(function () {
      ...
      $.post('?admin&changes', formData)
        ...
        .fail(function (xhr, status, error) {
          ...
          showAppModal({ type: 'danger', message: 'Erreur lors de la modification: ' + xhr.responseText });
        });
    });
